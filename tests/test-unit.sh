...
test_input_validation_edge_cases() {
  ...
  assert_failure "Tab-only second arg" "$MAIN_SCRIPT" "title1" 
...
' "body2"
  ...
  assert_success "Very long arguments" "$MAIN_SCRIPT" "$long_string" "$long_string" "$long_string" "$long_string"
  
  # Test with special characters
  assert_success "Special characters" "$MAIN_SCRIPT" "Title with spaces" "Body with\nnewlines" "Title-with-dashes" "Body with \"quotes\""
}
